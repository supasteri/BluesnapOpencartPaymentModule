...
class ControllerExtensionPaymentBluesnap extends Controller {
	...
	protected function getAuditTrail(&$data) {
		...
		$filter_data = array(
                ...
                foreach ($results as $result) {
                        $audit['entries'][] = array(
                                ...
                                'view'                  => $this->url->link('extension/payment/bluesnap/get_hosted_audit_entry', 'token=' . $this->session->data['token'] . '&bluesnap_hosted_fields_audit_id=' . $result['bluesnap_hosted_fields_audit_id'], 'SSL'),
                        );
                }
		...
		if ($order == 'ASC') {
                ...
                $audit['sort_order'] = $this->url->link('extension/payment/bluesnap', 'token=' . $this->session->data['token'] . '&sort=order_id' . $url, 'SSL');
                $audit['sort_outcome'] = $this->url->link('extension/payment/bluesnap', 'token=' . $this->session->data['token'] . '&sort=result_code' . $url, 'SSL');
                //$audit['sort_total'] = $this->url->link('extension/payment/bluesnap', 'user_token=' . $this->session->data['user_token'] . '&sort=amount' . $url, 'SSL');
                $audit['sort_date_added'] = $this->url->link('extension/payment/bluesnap', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, 'SSL');
                $audit['sort_remote_ip'] = $this->url->link('extension/payment/bluesnap', 'token=' . $this->session->data['token'] . '&sort=remote_ip' . $url, 'SSL');
                ...
                $pagination->url = $this->url->link('extension/payment/bluesnap', 'token=' . $this->session->data['token'] . $url . '&hostedpage={page}', 'SSL');
