...
class ControllerPaymentBluesnap extends Controller {
	...
	public function index() {
		...
		$data['action'] = $this->url->link('payment/bluesnap', 'token=' . $this->session->data['token'], 'SSL');
		...
		if (isset($this->request->post['bluesnap_username'])) {
			...
			if($this->config->get('bluesnap_mode')=="production"){
        			$data['bluesnap_username'] = $this->config->get('bluesnap_production_username');
			}else{
		...
		$data['bluesnap_server_ip'] =  "[Hostname:$hostname] [Public IP:$public_ip] [Internal IP: $internal_ip]";
                ...
                if (isset($this->request->post['bluesnap_password'])) {
                        ...
                        if($this->config->get('bluesnap_mode')=="production"){
        			$data['bluesnap_password'] = $this->config->get('bluesnap_production_password');
			}else{
		...
		$this->response->setOutput($this->load->view('payment/bluesnap.tpl', $data));
	}
	...
	protected function getAuditTrail(&$data) {
		...
		$filter_data = array(
                ...
                $entries_total = $this->model_payment_bluesnap->get_audit_total_entries($filter_data);//echo '11<pre>';print_r($entries_total);exit;

                $results = $this->model_payment_bluesnap->get_audit_entries($filter_data);//echo '11<pre>';print_r($results);exit;
                $audit['entries'] = array();
	...
	public function orderAction() {
		...
		$this->load->model('payment/bluesnap');
		$this->load->language('payment/bluesnap_order');
		...
		if (sizeof($bluesnap_audit_entries) > 0) {
			...
			return $this->load->view('payment/bluesnap_order.tpl', $data);
		}
	}
	...
	protected function getList(&$data) {
		...
		$data['sort_order'] = $this->url->link('payment/bluesnap', 'token=' . $this->session->data['token'] . '&sort=order_id' . $url, 'SSL');
		$data['sort_outcome'] = $this->url->link('payment/bluesnap', 'token=' . $this->session->data['token'] . '&sort=result_code' . $url, 'SSL');
		$data['sort_total'] = $this->url->link('payment/bluesnap', 'token=' . $this->session->data['token'] . '&sort=amount' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('payment/bluesnap', 'token=' . $this->session->data['token'] . '&sort=date_added' . $url, 'SSL');
		$data['sort_remote_ip'] = $this->url->link('payment/bluesnap', 'token=' . $this->session->data['token'] . '&sort=remote_ip' . $url, 'SSL');
